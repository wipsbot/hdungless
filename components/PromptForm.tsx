...
// FIX: Add missing IMAGE_TO_VIDEO mode and align icons with App.tsx for consistency.
const modeIcons: Record<GenerationMode, React.ReactNode> = {
  [GenerationMode.TEXT_TO_VIDEO]: <TextModeIcon className="w-5 h-5" />,
  [GenerationMode.IMAGE_TO_VIDEO]: <FramesModeIcon className="w-5 h-5" />,
  [GenerationMode.FRAMES_TO_VIDEO]: <RectangleStackIcon className="w-5 h-5" />,
  [GenerationMode.REFERENCES_TO_VIDEO]: (
    <ReferencesModeIcon className="w-5 h-5" />
  ),
  // FIX: Add missing CHARACTER_SYNC icon to fix build error.
  [GenerationMode.CHARACTER_SYNC]: <UsersIcon className="w-5 h-5" />,
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  useEffect(() => {
    if (
      ...
      generationMode === GenerationMode.CHARACTER_SYNC
    ) {
  ...
  // FIX: Add prompt placeholder for IMAGE_TO_VIDEO mode.
  const promptPlaceholder = {
    ...
    [GenerationMode.CHARACTER_SYNC]:
      'Describe a video using characters. Mention character names in prompt...',
    [GenerationMode.EXTEND_VIDEO]: 'Describe what happens next (optional)...',
  }[generationMode];

  // FIX: Add IMAGE_TO_VIDEO to selectable modes.
  ...
  // FIX: Handle media uploads for IMAGE_TO_VIDEO mode.
  const renderMediaUploads = () => {
    ...
    if (
      ...
      const isCharacterSyncMode =
        generationMode === GenerationMode.CHARACTER_SYNC;
      const maxImages = isCharacterSyncMode ? 10 : 3;
      const uploadLabel = isCharacterSyncMode ? 'Add Character' : 'Add Reference';
      return (
  ...
  // FIX: Handle form submission validation for IMAGE_TO_VIDEO mode.
  switch (generationMode) {
    ...
    case GenerationMode.CHARACTER_SYNC:
      ...
      if (hasNoRefs && hasNoPrompt) {
        tooltipText = 'Please add image(s) and enter a prompt.';
      } else if (hasNoRefs) {
        tooltipText =
          generationMode === GenerationMode.CHARACTER_SYNC
            ? 'At least one character image is required.'
